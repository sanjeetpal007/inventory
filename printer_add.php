...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO printer (
            VENDOR_NAME, INVOICE_NO, BILL_DATE, SN, OUTSTATION,
            PRINTER_MODEL, PRINTER_TYPE, PRINTER_SERIAL_NUMBER,
            CORRECT_SERIAL_NUMBER, INVENTORY, WORKSTATION, USER_NAME,
            FLOOR_NUMBER, USER_ID
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        $stmt->execute([
            $_POST['VENDOR_NAME'], $_POST['INVOICE_NO'], $_POST['BILL_DATE'], $_POST['SN'],
            $_POST['OUTSTATION'], $_POST['PRINTER_MODEL'],  $_POST['PRINTER_TYPE'],
            $_POST['PRINTER_SERIAL_NUMBER'], $_POST['CORRECT_SERIAL_NUMBER'], $_POST['INVENTORY'],
            $_POST['WORKSTATION'], $_POST['USER_NAME'], $_POST['FLOOR_NUMBER'], $_POST['USER_ID']
        ]);

        echo "<script>
            alert('✅ Printer added successfully!');
            window.location.href = 'printer_list.php';
        </script>";
        exit();
    } catch (PDOException $e) {
        echo "<script>
            alert('❌ Unexpected error while saving the Printer.');
            window.history.back();
        </script>";
        exit();
    }
  ...
  <div class="form-container">
    <form id="pcAddForm" method="POST">
      <?php
        ...
        $requiredFields = [
          'VENDOR_NAME', 'INVOICE_NO', 'BILL_DATE', 'SN', 'OUTSTATION',
          'PRINTER_MODEL', 'PRINTER_TYPE', 'PC_SERIAL_NUMBER', 'USER_ID'
        ];
        foreach ($fields as $field):
          $isRequired = in_array($field, $requiredFields) ? 'required' : '';
		  $isHalf = in_array($field, [ 'INVOICE_NO', 'BILL_DATE', 'OUTSTATION','PRINTER_MODEL']) ? 'half' : '';
      ?>
